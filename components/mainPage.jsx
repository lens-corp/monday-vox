...
export default function MainPage() {
  ...
  const handlePrompt = async (prompt) => {
    let result = await axios
      .post("https://73ab-103-234-71-66.ngrok-free.app/getText", {
        query: prompt,
      })
      .then((response) => {
  ...
  const sendData = async (name, datetime, file) => {
    ...
    try {
      ...
      let commandResult = await handlePrompt(data);
      ...
      if (commandResult) {
        ...
        let u = new SpeechSynthesisUtterance(commandResult);
        u.rate = 0.8;
        window.speechSynthesis.speak(u);
      }
    } catch (err) {
  ...
  return (
    <div className={classes.container}>
      ...
      {openSettings &&
        createPortal(<Dialog onClose={handleCloseSettings} />, document.body)}
      {openSnack &&
        createPortal(
          <SnackBar
            onClose={handleCloseSnack}
            text="Problem sending the request to remote Whisper API."
          />,
          document.body
        )}
      {openAudioDialog &&
      ...
      {openModal &&
        createPortal(
          <Modal
            ...
            buttonText="Delete"
            onButtonClick={handleClickModal}
